...
public class AppContext
{
    ...
    private async Task ProcessClipboardText(string text)
    {
        ...
        var promptName = Config.GetCurrentPromptName() ?? "未知";
        ...
        try
        {
            if (Config.Data.ShowNotifications)
                _tray.ShowNotification($"正在使用「{promptName}」处理...");
            else
                _tray.SetStatusText($"正在处理 [{promptName}]...");
            ...
            if (result != null)
            {
                ...
                _tray.SetStatusText($"完成: {preview} ({_history.Count}条)");
                if (Config.Data.ShowNotifications)
                    _tray.ShowNotification($"处理完成: {preview}");
            }
            else
            {
                entry.Status = EntryStatus.Failed;
                entry.ErrorMessage = "AI 返回空结果";
                _tray.SetStatusText($"处理失败: AI 返回空结果 ({_history.Count}条)");
                if (Config.Data.ShowNotifications)
                    _tray.ShowNotification("AI 返回空结果", ToolTipIcon.Error);
            }
        }
        catch (OperationCanceledException)
        {
            entry.Status = EntryStatus.Failed;
            entry.ErrorMessage = "处理超时";
            _tray.SetStatusText($"处理超时已取消 ({_history.Count}条)");
            if (Config.Data.ShowNotifications)
                _tray.ShowNotification("处理超时已取消", ToolTipIcon.Warning);
        }
        catch (Exception ex)
        {
            entry.Status = EntryStatus.Failed;
            entry.ErrorMessage = ex.Message;
            _tray.SetStatusText($"处理失败: {ex.Message} ({_history.Count}条)");
            if (Config.Data.ShowNotifications)
                _tray.ShowNotification($"处理失败: {ex.Message}", ToolTipIcon.Error);
        }
        finally
        {
            _processing = false;
    ...
    public void ToggleEnabled()
    {
        ...
        Config.Save();
        _tray.UpdateEnableState();
        _tray.ShowNotification(IsEnabled ? "已启用" : "已禁用");
    }
    ...
    public void SelectPrompt(string id)
    {
        ...
        OnConfigChanged();
        _tray.ShowNotification($"已切换至「{Config.GetCurrentPromptName()}」");
    }
