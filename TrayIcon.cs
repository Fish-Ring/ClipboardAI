...
public class TrayIcon : IDisposable
{
    ...
    private void BuildMenu()
    {
        ...
        _enableItem = new ToolStripMenuItem("已启用") { Checked = _context.IsEnabled };
        _enableItem.Click += (s, e) => _context.ToggleEnabled();
        _menu.Items.Add(_enableItem);

        _menu.Items.Add(new ToolStripSeparator());

        var promptItem = new ToolStripMenuItem("Prompt 预设");
        promptItem.DropDownOpening += (s, e) => RebuildPromptSubmenu(promptItem);
        _menu.Items.Add(promptItem);

        var historyItem = new ToolStripMenuItem("最近处理");
        historyItem.DropDownOpening += (s, e) => RebuildHistorySubmenu(historyItem);
        _menu.Items.Add(historyItem);

        _menu.Items.Add(new ToolStripSeparator());

        var editItem = new ToolStripMenuItem("编辑配置");
        editItem.Click += (s, e) => _context.ShowConfigForm();
        _menu.Items.Add(editItem);

        var openItem = new ToolStripMenuItem("打开配置文件");
        openItem.Click += (s, e) => OpenConfigFile();
        _menu.Items.Add(openItem);

        var openFolderItem = new ToolStripMenuItem("打开配置文件夹");
        openFolderItem.Click += (s, e) => OpenConfigFolder();
    ...
    private void RebuildHistorySubmenu(ToolStripMenuItem parent)
    {
        ...
        if (entries.Count == 0)
        {
            parent.DropDownItems.Add(new ToolStripMenuItem("(空)") { Enabled = false });
            return;
        }
